# Firefox Browser Agent Bridge — Reference

Detailed examples and workflows for advanced actions. See [SKILL.md](SKILL.md) for the quick-start guide and action reference tables.
...
## Cross-Origin Iframe Workflow

Many login pages (Apple, Google, Microsoft) embed their sign-in forms inside cross-origin iframes. Standard `click`/`type`/`getContent` only target the main frame by default.
